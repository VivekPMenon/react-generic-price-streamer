...
export function ResearchReports({ isExpanded }: ResearchReportsProps) {
  const reports = [
    ...
    'BNP Paribas Exane Research - 2025 - The Pricey Paradise'
  ];

  const aiSummary = `# Market Overview
...
`;
  ...
  const emailContent = `# Ruminations - Macro, Micro, Markets
    ...
    <div className={styles['research-reports']}>
      ...
      <div className={styles['report-list']}>
        ...
        <div className='news'>
          {
            reports.map(report =>
              <div className={report === selectedNews ? 'news-item active' : 'news-item'}
        ...
        isSummaryVisible ?
          <>
            ...
            <div className={`${styles['ai-summary']} scrollable-div ${isExpanded ? styles['expanded'] : ''}`}>
              ...
              <ReactMarkdown className='markdown' remarkPlugins={[remarkGfm]}>{aiSummary}</ReactMarkdown>
            </div>
          </> : <></>
      }
